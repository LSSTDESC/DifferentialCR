...
def find_average_wavelength(filter_file, wavelengths, directory, cutoff = 8005):
    ...
    fwhm, left_wavelength, right_wavelength, left_idx, right_idx = calculate_fwhm2(wavelength, throughput)
...
def calculate_filter_refractions(filter_files, directory, airmasses, dcr = True, cutoff = 8005):
    ...
    wavelengths = np.linspace(300, 1100, 10000) #len(airmasses))
    zeniths = []
...
def calculate_filter_refractions2(filter_files, directory, airmasses, dcr = True, cutoff = 8005):
    ...
    # Get the edge of each filter using the filter library and store them in a single array
    filter_library = get_filter_library()

    wavelength_edges = np.zeros(len(filter_library) * 2)
    filter_order = []
    counter = 0
    
    for filter_label, filter_data in filter_library.items():
        wavelength = filter_data.wavelen[: (cutoff + 1)]
        throughput = filter_data.sb[: (cutoff + 1)]

        fwhm, left_wavelength, right_wavelength, left_idx, right_idx = calculate_fwhm2(wavelength, throughput)

        wavelength_edges[counter] = left_wavelength
        wavelength_edges[counter + 1] = right_wavelength
        filter_order.append(filter_label)

        counter += 1


    #calculate the refraction angle at each zenith and wavelength_edge
    gs_refractions = np.zeros((len(zeniths), len(wavelength_edges)))
    for i, zenith in enumerate(zeniths):
        gs_refraction = 3600 * np.rad2deg(galsim.dcr.get_refraction(
            wavelength_edges,
            zenith_angle=zenith.to_value(u.deg) * galsim.degrees,
    ...
    #create a library in the following format {filter : [array of differential refraction angles at each zenith]}
    # filter_refractions = {}
    filter_refractions = []
    for i, filt in enumerate(filter_order):
        refraction_left = gs_refractions[:, i]
        refraction_right = gs_refractions[:, i + 1]
        diff_refraction = abs(refraction_left - refraction_right) if dcr else np.zeros_like(airmasses)
        # filter_refractions[filt] = diff_refraction
        filter_refractions.append(diff_refraction)
...
def calculate_fwhm(wavelength, throughput, cutoff = 8005): #this could be optimized
...
def calculate_fwhm2(wavelength, throughput, cutoff = 8005):
    max_index = np.argmax(throughput)
    half_max = throughput[max_index] / 10 #may change to actual half maximum. This will calculate the range of the filter

    #get left index
    left_idx = np.where(throughput[:max_index] < half_max)[0] #all indices left of the max under the half_max
    if len(left_idx) == 0:
        raise ValueError("Left half maximum crossing not found.")
    left_idx = left_idx[-1]
    left_wave = wavelength[left_idx]

    #get right index 
    right_idx = np.where(throughput[max_index:] < half_max)[0]
    if len(right_idx) == 0:
        raise ValueError("Right half maximum crossing not found.")
    right_idx = right_idx[0] + max_index
    right_wave = wavelength[right_idx]

    fwhm = right_wave - left_wave
    return fwhm, left_wave, right_wave, left_idx, right_idx
...
def calculate_refraction_and_seeing_effects(filter_files, directory, airmasses, calc_type = None, airmass_seeing = True, 
    ...
    for i, filter_file in enumerate(filter_files):
        ...
        nominal_wavelength = find_average_wavelength('total_i.dat', wavelengths, directory, cutoff = 8005)#filter_files[3], wavelengths, directory, cutoff = 8005)
        left_idx_new = find_nearest_index(wavelengths, left_wavelength)
        right_idx_new = find_nearest_index(wavelengths, right_wavelength)
        avg_wavelength = (left_wavelength + right_wavelength) / 2
        avg_idx_new = find_nearest_index(wavelengths, avg_wavelength)
        ...
        filter_refractions = calculate_filter_refractions2(filter_files, directory, dcr = dcr, airmasses = airmasses, cutoff = 8005)
...
def calculate_quartile(opsim_fname, fraction = 0.5, filter = None):
    ...
    '''
    Not sure why we're getting all the ra and dec coordinates from the file then using those to match the pixel numbers from the 
    cosmology cuts when we could just convert the pixel numbers to ra and dec directly?
...
def get_filter_library():
    return rubin_sim.phot_utils.rubin_bandpasses()
...
# Create a figure for the subplots (5 rows, 3 columns)
fig, axs = plt.subplots(nrows=1, ncols=3, figsize=(10, 3.2)) #, constrained_layout=False) #12, 10 may be the best

fig.suptitle("DCR Ellipticity in the g Band", fontsize = 20)
opsim_fname = get_baseline() #Look at this, didn't work 
print(opsim_fname)
# opsim_fname = '/Users/msredden/Documents/DESC/DifferentialCR/db files/baseline_v4.3.2_10yrs.db'
...
for i, filter in enumerate(filters):
    # Dictionary to map filter letters to their index
    filter_map = {'u': 0, 'g': 1, 'r': 2, 'i': 3}
    filter_file_index = 0 #filter_map.get(filter, 0) #return the index of the filter if in filter_map, otherwise return 0 --> u?
    ...
    for j, nth_quartile in enumerate(quartiles):
        ...
        # Plot the HEALPix map using mollview
        hp.mollview(g_shear[filter_file_index], title=f"Percentile: {nth_quartile*100}%", 
                    cmap='magma', cbar=False, nest=True, min=0, max=0.16, hold=True)

        if max(g_shear[filter_file_index]) > vmax:
            vmax = max(g_shear[filter_file_index])
...
plt.tight_layout()
plt.show()
fig.savefig('DESC_meeting_map.pdf')
